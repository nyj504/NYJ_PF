...
bool BoxCollider::IsBoxCollision(BoxCollider* collider, Vector3* overlap)
{
    if (!IsActive()) return false;

    if (overlap)
        return IsAABB(collider, overlap);

    return IsOBB(collider);
}

bool BoxCollider::IsAABB(BoxCollider* collider , Vector3* overlap)
{
    if (!IsActive()) return false;

    Vector3 thisMin = GetGlobalPosition() - HalfSize(); 
    Vector3 thisMax = GetGlobalPosition() + HalfSize();  

    Vector3 otherMin = collider->GetGlobalPosition() - collider->HalfSize();
    Vector3 otherMax = collider->GetGlobalPosition() + collider->HalfSize(); 
    ...
    if (overlap)
    {
        overlap->x = min(thisMax.x, otherMax.x) - max(thisMin.x, otherMin.x);
        overlap->y = min(thisMax.y, otherMax.y) - max(thisMin.y, otherMin.y);
        overlap->z = min(thisMax.z, otherMax.z) - max(thisMin.z, otherMin.z);
    }
    return true; 
}

bool BoxCollider::IsOBB(BoxCollider* collider)
{
    if (!IsActive()) return false;
