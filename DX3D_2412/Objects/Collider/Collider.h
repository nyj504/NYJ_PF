...
public:
    ...
    void Render();
    void RenderInstanced(UINT count, D3D11_PRIMITIVE_TOPOLOGY type);


    bool IsCollision(Collider* collider, Vector3* overlap = nullptr);

    virtual bool IsRayCollision(IN const Ray& ray, OUT RaycastHit* hit = nullptr) = 0;
    virtual bool IsBoxCollision(class BoxCollider* collider, Vector3* overlap) = 0;
