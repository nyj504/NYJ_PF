...
void SubChunk::FindVisibleBlocks()
{
	...
	for (const pair<UINT64, Block*> pair : blocks)
	{
		...
		block->SetOcclusion(false); // ∫Ì∑œ¿Ã «— πÊ¿ß∂Ûµµ ≥Î√‚µ«∏È ¿ŒΩ∫≈œΩ∫ µ•¿Ã≈Õ √ﬂ∞°
		...
		visibleInstanceData.index = block->GetBlockInstanceID(); //∫Ì∑œ ∞Ì¿Ø¿« ¿ŒΩ∫≈œΩ∫ æ∆¿Ãµ 
		visibleInstanceData.isActive = block->IsActive();
					
		if (block->IsNormal()) // ∏µÁ ∏È¿Ã µø¿œ«— ∫Ì∑∞ 
