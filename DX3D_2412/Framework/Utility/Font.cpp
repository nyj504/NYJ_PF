...
void Font::RenderText(wstring text, Vector2 pos, Vector2 size)
{
	...
	curFormat->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_CENTER);

	pos.y = SCREEN_HEIGHT - pos.y;

	D2D1_RECT_F rectF;
	rectF.left = pos.x - (size.x * 0.5f);
	rectF.right = pos.x + (size.x * 0.5f);
	rectF.top = pos.y - (size.y * 0.5f);
	rectF.bottom = pos.y + (size.y * 0.5f);

	context->DrawTextW(text.c_str(), text.size(),
		curFormat, &rectF, curBrush);
}

void Font::RenderText(string text, Vector2 pos, Vector2 size)
{
	if (size.x == 0.0f && size.y == 0.0f)
	{
		size.x = text.size() * curFormat->GetFontSize();
